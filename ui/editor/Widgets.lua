...
GUI:RegisterWidgetType("APRStepRow", function()
    ...
    function widget:LayoutDescription(width)
        detail:SetWidth(math.max(1, (width or frame:GetWidth()) - 49))
        detail:SetHeight(0)
        local height = math.max(16, detail:GetStringHeight())
        detail:SetHeight(height)
        meta:ClearAllPoints()
        meta:SetPoint("TOPLEFT", 40, -35 - height)
        meta:SetPoint("RIGHT", -9, 0)
        self:SetHeight(56 + height)
    end
    function widget:OnWidthSet(width)
        self:LayoutBadges(width)
        self:LayoutDescription(width)
    end
    function widget:OnAcquire()
        detail:SetText("")
        self:SetHeight(72)
