...
assert(#session.draft.steps[1].Class == 1 and session.draft.steps[1].Class[1] == 13)
GUI:ClearFocus()
...
assert(#session.draft.steps[1].Class == 1 and session.draft.steps[1].Class[1] == 8)
assert(E:Save())

-- Route and parallel conditions use the same multi-select and save valid lists.
E:SelectTab("route")
assert(find(E.routeForm, function(w) return w:GetUserData("navigateKey") == "conditions" end)):Fire("OnClick")
